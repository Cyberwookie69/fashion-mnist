"""3D wiremesh plot of sigmoid test accuracy across (epochs x batch_size).

If matrix_results.csv exists with Sigmoid rows, uses those. Otherwise runs a
sigmoid-only mini-sweep (100 cells, ~3-8 min) and writes sigmoid_sweep.csv.

Each wire segment is shaded from light gray (low accuracy) to black (highest
accuracy), so the optimum visually pops out as the darkest region.
"""
...
def get_sigmoid_data() -> pd.DataFrame:
    if MATRIX_CSV.exists():
        ...
        sub = df[df["model"] == "Sigmoid"]
        if len(sub) > 0:
            print(f"using {MATRIX_CSV.name}: {len(sub)} sigmoid cells")
            return sub
    if SIGMOID_CSV.exists():
        sub = pd.read_csv(SIGMOID_CSV)
        print(f"using {SIGMOID_CSV.name}: {len(sub)} sigmoid cells")
        return sub
    print("no cached data; running sigmoid-only sweep (~3-8 min)...")
    ...
    df.to_csv(SIGMOID_CSV, index=False)
    print(f"wrote {SIGMOID_CSV.name} ({len(df)} cells, {dt:.1f}s)")
    return df


def main() -> None:
    sub = get_sigmoid_data()
    ...
    ax.set_title("Sigmoid test accuracy over (epochs x batch_size)\n"
